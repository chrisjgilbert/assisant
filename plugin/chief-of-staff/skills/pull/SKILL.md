---
name: pull
description: Connector-driven fetch of Slack and Google Calendar into the immutable raw layer (~/sources/). Use to refresh raw sources before an ingest (e.g. at the start of the daily loop) or on demand when newer source data is needed. Read-only on Slack; never sends or posts anything.
---
...
the **immutable raw layer** at `~/sources/`. This is the "fetcher into raw," NOT
...
- **Writes are confined to:** `~/sources/**`, `~/brain/signals/new-channels.md`,
  and `~/brain/log.md`. Nothing else.
- **NEVER write** `~/brain/goals/`, `~/brain/context/`, or `~/brain/index.md`.
...
## Inputs it reads
- `~/brain/config/sources.yml` — the source/channel selection. Schema:
  ```yaml
  slack:
  ...
  calendar:
    ...
    calendars: ["primary"]
  ```
- `~/brain/log.md` — to determine which days/windows are already captured
  (idempotency).
- Existing files under `~/sources/` — to avoid duplicating captured days.

## Procedure
1. **Load config.** Read `~/brain/config/sources.yml`. Resolve effective windows:
...
2. **Compute idempotency.** From `~/brain/log.md` and existing `~/sources/` files,
...
5. **Write Slack raw snapshots.** Per channel/DM, write
   `~/sources/slack/<channel-or-dm-slug>/YYYY-MM-DD.md` (and/or `.json` for the
...
6. **Fetch + write Calendar.** For the calendars in `calendar.calendars`, fetch
   ...
   `~/sources/calendar/YYYY-MM-DD.md` with frontmatter.
7. **Channel discovery.** List the channels you are a **member** of. Diff against
   ...
   `~/brain/signals/new-channels.md` (see format). NEVER auto-add to `include`.
...
### Slack raw — `~/sources/slack/<slug>/YYYY-MM-DD.md` (+ optional `.json`)
```yaml
---
source: slack
...
### Calendar raw — `~/sources/calendar/YYYY-MM-DD.md`
```yaml
---
source: calendar
calendars: ["primary"]
...
### Channel discovery — append to `~/brain/signals/new-channels.md`
...
- Write `~/sources/slack/eng-product/2026-06-25.md`,
  `~/sources/slack/incidents/2026-06-25.md`, DM snapshots, and
  `~/sources/calendar/2026-06-25.md`.
